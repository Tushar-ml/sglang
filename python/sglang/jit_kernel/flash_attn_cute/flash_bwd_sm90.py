...
from sglang.jit_kernel.flash_attn_cute import barrier, pipeline, utils
...
from sglang.jit_kernel.flash_attn_cute.named_barrier import NamedBarrierBwd
...
class FlashAttentionBackwardSm90:
    ...
    def _get_tiled_mma(self):
        maybe_swap_mn = lambda shape, swap: (
            (shape[1], shape[0], *shape[2:]) if swap else shape
        )
        # S = Q @ K.T, dP = dO @ V.T
        atom_layout_SdP = (
            self.AtomLayoutMSdP,
    ...
    def mma_one_m_block(
        ...
        consumer_state_dO: (
            cutlass.pipeline.PipelineState | pipeline.PipelineStateSimple
        ),
