...
from sglang.jit_kernel.flash_attn_cute.cache_utils import get_jit_cache
from sglang.jit_kernel.flash_attn_cute.testing import is_fake_mode

if os.environ.get("CUTE_DSL_PTXAS_PATH", None) is not None:
    from sglang.jit_kernel.flash_attn_cute import cute_dsl_ptxas  # noqa: F401

    # Patch to dump ptx and then use system ptxas to compile to cubin
    cute_dsl_ptxas.patch()


from sglang.jit_kernel.flash_attn_cute import fa_logging, utils
from sglang.jit_kernel.flash_attn_cute.block_sparsity import (
...
from sglang.jit_kernel.flash_attn_cute.cute_dsl_utils import (
    get_aux_tensor_metadata,
    get_broadcast_dims,
    to_cute_aux_tensor,
    to_cute_tensor,
)
from sglang.jit_kernel.flash_attn_cute.flash_bwd import FlashAttentionBackwardSm80
from sglang.jit_kernel.flash_attn_cute.flash_bwd_postprocess import (
    FlashAttentionBackwardPostprocess,
)
from sglang.jit_kernel.flash_attn_cute.flash_bwd_preprocess import (
    FlashAttentionBackwardPreprocess,
)
from sglang.jit_kernel.flash_attn_cute.flash_bwd_sm90 import FlashAttentionBackwardSm90
from sglang.jit_kernel.flash_attn_cute.flash_bwd_sm100 import (
    FlashAttentionBackwardSm100,
)
from sglang.jit_kernel.flash_attn_cute.flash_bwd_sm120 import (
    FlashAttentionBackwardSm120,
)
from sglang.jit_kernel.flash_attn_cute.flash_fwd import FlashAttentionForwardSm80
from sglang.jit_kernel.flash_attn_cute.flash_fwd_combine import (
    FlashAttentionForwardCombine,
)
from sglang.jit_kernel.flash_attn_cute.flash_fwd_mla_sm100 import (
    FlashAttentionMLAForwardSm100,
)
from sglang.jit_kernel.flash_attn_cute.flash_fwd_sm90 import FlashAttentionForwardSm90
from sglang.jit_kernel.flash_attn_cute.flash_fwd_sm100 import (
    DescaleTensors,
    FlashAttentionForwardSm100,
)
from sglang.jit_kernel.flash_attn_cute.flash_fwd_sm120 import FlashAttentionForwardSm120
from sglang.jit_kernel.flash_attn_cute.sm100_hd256_2cta_fmha_backward import (
    BlackwellFusedMultiHeadAttentionBackward,
)

# SM100 head_dim=256 2CTA kernel imports
from sglang.jit_kernel.flash_attn_cute.sm100_hd256_2cta_fmha_forward import (
...
def _flash_attn_fwd(
    ...
    if qv is not None:
        ...
        assert (
            head_dim == 64 and head_dim_v == 512
        ), "only support MLA weight absorbed shape with qv"
...
class FlashAttnVarlenFunc(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout, dlse):
        q, k, v, out, lse, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k, *aux = (
            ctx.saved_tensors
        )
        aux_tensors = aux if aux else None
        if not ctx.return_lse:
