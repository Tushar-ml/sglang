...
from sglang.jit_kernel.flash_attn_cute import barrier, copy_utils, pipeline, utils
from sglang.jit_kernel.flash_attn_cute.blackwell_helpers import (  # noqa
    gemm_ptx_w_idx,
    gemm_w_idx,
)
...
from sglang.jit_kernel.flash_attn_cute.named_barrier import NamedBarrierBwdSm100
...
from sglang.jit_kernel.flash_attn_cute.tile_scheduler import (  # noqa
...
class FlashAttentionBackwardSm100:
    ...
    def __call__(
        ...
        tile_sched_args = TileSchedulerArguments(
            ...
            total_q=(
                cute.size(mK.shape[0])  # pass total_k for total_q
    ...
    def kernel(
        ...
        if const_expr(self.use_2cta_instrs):
            if const_expr(self.tile_hdim <= 128):
                ...
                sdS_xchg = storage.sdQaccum.get_tensor(
    ...
    def dQacc_reduce(
        ...
        expected_reduce_stages_t2r = (
            self.dQaccum_reduce_stage_t2r // self.cta_group_size
        )
        assert (
        ...
        while work_tile.is_valid_tile:
            ...
            for iter_idx in cutlass.range(loop_count, unroll=1):
                ...
                if const_expr(self.deterministic and not delay_semaphore_release):
                    if const_expr(
                        self.sdQaccum_stage > 1 and not self.tile_hdim == 192
                    ):
                        if is_tma_warp and not m_block_oob_upper:
                            cute.arch.cp_async_bulk_wait_group(0, read=read_flag)
                        self.reduce_sync_barrier.arrive_and_wait()
