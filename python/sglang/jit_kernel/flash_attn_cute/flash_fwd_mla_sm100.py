...
import sglang.jit_kernel.flash_attn_cute.blackwell_helpers as fa_sm100_utils
from sglang.jit_kernel.flash_attn_cute.block_info import BlockInfo
from sglang.jit_kernel.flash_attn_cute.cute_dsl_utils import dump_kernel_attributes
from sglang.jit_kernel.flash_attn_cute.fa_logging import fa_log, fa_printf
from sglang.jit_kernel.flash_attn_cute.mask import AttentionMask
from sglang.jit_kernel.flash_attn_cute.named_barrier import NamedBarrierFwdSm100_MLA2CTA
from sglang.jit_kernel.flash_attn_cute.pack_gqa import (
    make_packgqa_tiled_tma_atom,
    pack_gqa_layout,
)
from sglang.jit_kernel.flash_attn_cute.seqlen_info import SeqlenInfoQK
from sglang.jit_kernel.flash_attn_cute.softmax import SoftmaxSm100
from sglang.jit_kernel.flash_attn_cute.testing import attention_ref
from sglang.jit_kernel.flash_attn_cute.tile_scheduler import (
    ClcState,
    ParamsBase,
    SchedulingMode,
...
from sglang.jit_kernel.flash_attn_cute.topk_gather_kv import CpasyncGatherKVManager
from sglang.jit_kernel.flash_attn_cute.utils import smid
...
class FlashAttentionMLAForwardSm100:
    ...
    def __call__(
        ...
        mLSE: Optional[
            cute.Tensor
        ],  # (b, h, s_q) or (h, total_q) if there is cu_seqlens_q
        ...
        mQ, mQv, mO = [
            cute.make_tensor(
                mX.iterator, cute.select(mX.layout, mode=QO_layout_transpose)
            )
            for mX in (mQ, mQv, mO)
        ]
        mK, mV = [
            cute.make_tensor(
                mX.iterator, cute.select(mX.layout, mode=KV_layout_transpose)
            )
            for mX in (mK, mV)
        ]
        ...
        if const_expr(self.pack_gqa):
            ...
            if const_expr(mLSE is not None):
                mLSE = pack_gqa_layout(
                    mLSE, self.qhead_per_kvhead, self.nheads_kv, head_idx=1
                )
        ...
        def split_hdimv(m, dim: int):
            ...
            split_stride = (
                *m.layout.stride[:dim],
                (1, chunk),
                *m.layout.stride[dim + 1 :],
            )
            split = cute.make_tensor(
                m.iterator, cute.make_layout(split_shape, stride=split_stride)
            )
        ...
        self.sScale_layout = cute.make_layout(
            (self.cta_tile_m, self.num_stages_sm_stats)
        )
        self.sBitmask_layout = cute.make_layout(
            (self.tile_n // 32, self.num_stages_bitmask)
        )
        ...
        def make_tma(make_fn, mX, smem_layout, mma_tiler, tiled_mma):
            return make_fn(
                tma_load_op, mX, smem_layout, mma_tiler, tiled_mma, cta_shape
            )
    ...
    def kernel(
        ...
        sm_threads = pipeline.CooperativeGroup(
            pipeline.Agent.Thread, self.num_softmax_threads
        )
        epi_threads = pipeline.CooperativeGroup(
        ...
        if const_expr(self.overlap_sO_sV):
            ...
            assert (
                ...
                <= cute.cosize(sVi_layout_staged) * self.num_hdimv_splits
            )
        else:
            sO_iterator = sQv0.iterator
            assert (
                cute.cosize(sO_layout)
                <= cute.cosize(sQvi_layout_staged) * self.num_hdimv_splits
            )
        sO = cute.make_tensor(
    ...
    def load_cpasync(
        ...
        while work_tile.is_valid_tile:
            ...
            mV0_cur = seqlen.offset_batch_K(mV0, batch_idx, dim=3)[
                None, None, head_idx_kv
            ]
            mV1_cur = seqlen.offset_batch_K(mV1, batch_idx, dim=3)[
                None, None, head_idx_kv
            ]
            # (hdimv//2, seqlen_k)
            ...
            # (hdimv//4, seqlen_k)
            hdimv_split_per_cta = (
                self.hdimv // self.num_hdimv_splits // self.cta_group_size
            )
            ...
            for n_block_group in cutlass.range(num_n_block_groups - 1, unroll=1):
                for stage in cutlass.range_constexpr(self.num_stages_S):
                    ...
                    # K, V0, V1
                    cpasync_gather_kv_manager.load_index_topk(
                        n_block + 1, transpose=False
                    )
                    producer_state_K = load_K(producer_state_K)
                    producer_state_V0 = load_V0(producer_state_V0)
                    producer_state_V1 = load_V1(producer_state_V1)
    ...
    def load(
        ...
        while work_tile.is_valid_tile:
            ...
            mQvs_cur = [
                seqlen.offset_batch_Q(mQvs[split], batch_idx, dim=3)[
                    None, None, head_idx
                ]
            ...
            if const_expr(self.use_tma_KV):
                ...
                mVs_cur = [
                    seqlen.offset_batch_K(mVs[split], batch_idx, dim=3)[
                        None, None, head_idx_kv
                    ]
                    for split in range(self.num_hdimv_splits)
                ]
            ...
            load_Qv = partial(
                self.load_inner, tma_atom_Qvs, tQvgQvs, tQvsQvs, pipeline_Qvs
            )
            if const_expr(self.use_tma_KV):
                load_K = partial(self.load_inner, tma_atom_K, tKgK, tKsK, pipeline_K)
                load_V = partial(
                    self.load_inner, tma_atom_Vs, tVgVs, tVsVs, pipeline_Vs
                )
                load_Vt = partial(
                    self.load_inner, tma_atom_Vts, tVtgVts, tVtsVts, pipeline_Vs
                )
            ...
            if const_expr(self.use_tma_KV):
                ...
                producer_state_V0 = load_V(
                    producer_state_V0, n_block=n_block_first, split=0
                )
                producer_state_V1 = load_V(
                    producer_state_V1, n_block=n_block_first, split=1
                )
                ...
                for n_block_group in cutlass.range(num_n_block_groups - 1, unroll=1):
                    for stage in cutlass.range_constexpr(self.num_stages_S):
                        n_block = (
                            n_block_max - 1 - n_block_group * self.num_stages_S - stage
                        )
                        # copy K gmem -> smem
                        producer_state_K = load_K(producer_state_K, n_block=n_block - 1)
                        # copy Vi gmem -> smem
                        producer_state_V0 = load_V(
                            producer_state_V0, n_block=n_block - 1, split=0
                        )
                        producer_state_V1 = load_V(
                            producer_state_V1, n_block=n_block - 1, split=1
                        )
                        # copy Vti gmem -> smem
                        producer_state_V0 = load_Vt(
                            producer_state_V0, n_block=n_block, split=0
                        )
                        producer_state_V1 = load_Vt(
                            producer_state_V1, n_block=n_block, split=1
                        )

                # ==== Epilogue ====
                num_final_n_blocks = (
                    self.num_stages_S if even_n_blocks else self.num_stages_S - 1
                )
    ...
    def softmax_loop(
        ...
        tSAcc_staged = [
            tStS[(None, None), 0, 0, stage] for stage in range(self.num_stages_S)
        ]
        ...
        tStS_t2r_staged = [
            tmem_load_thr.partition_S(tSAcc_staged[stage])
            for stage in range(self.num_stages_S)
        ]
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(
                (self.is_causal or self.is_local) and not self.is_topk_gather
            ):
                ...
                for stage in cutlass.range_constexpr(self.num_stages_S):
                    (
                        ...
                        mask_fn=(
                            ...
                            if const_expr(
                                self.is_topk_gather and not self.disable_bitmask
                            )
                            else None
                        ),
                    )
            ...
            sRowSum[tidx % self.cta_tile_m, warp_idx // self.cta_group_size] = (
                softmax.row_sum[0]
            )
            if const_expr(mLSE is not None):
    ...
    def softmax_step(
        ...
        if const_expr(self.is_topk_gather and not self.disable_bitmask):
            ...
            for i in cutlass.range_constexpr(cute.size(rBitmask)):
                rBitmask[i] = sBitmask[
                    bitmask_col_offset + i, consumer_state_bitmask.index
                ]
    ...
    def correction_loop(
        ...
        thr_tmem_load_O = tcgen05.make_tmem_copy(tmem_load_atom_O, tO0tO0).get_slice(
            tidx
        )
        thr_tmem_store_O = tcgen05.make_tmem_copy(tmem_store_atom_O, tO0tO0).get_slice(
            tidx
        )
    ...
    def correction_rescale(
        ...
        tOrO_t2r_frg = cute.make_rmem_tensor_like(
            tOcO_t2r[None, None, 0], self.dtype_acc
        )
...
def test_mla_kernel(
    ...
    if compile_key not in compile_cache:
        ...
        if varlen_q:
            ...
            lse = torch.empty(
                batch, nheads, seqlen_q, dtype=torch.float32, device="cuda"
            )
            index_topk = (
