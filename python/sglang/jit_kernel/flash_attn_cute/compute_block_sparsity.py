...
from sglang.jit_kernel.flash_attn_cute.utils import (
    hash_callable,
    scalar_to_ssa,
    ssa_to_scalar,
)
...
class BlockSparsityKernel:
    ...
    def __call__(
        ...
        self.mask_cnt, self.mask_idx, self.full_cnt, self.full_idx, *_ = (
            blocksparse_tensors
        )

        if const_expr(self.compute_full_blocks):
            assert (
                self.full_cnt is not None and self.full_idx is not None
