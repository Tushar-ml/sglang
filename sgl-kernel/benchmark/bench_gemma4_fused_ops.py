"""Benchmark CUDA kernels vs Triton for Gemma4 fused ops.
...
# Import Triton fallback implementations directly from the source module
...
def bench_rmsnorm(M, N, provider):
    ...
    if provider == "cuda":
        fn = lambda: torch.ops.sgl_kernel.gemma4_rmsnorm_residual_scalar.default(
            x, w, r, scalar, eps
        )
    else:
...
# Flatten the nested tuple for triton benchmark
QKV_FLAT = (
    ...
    if provider == "cuda":
        ...
        def fn():
            ...
            torch.ops.sgl_kernel.gemma4_qkv_rmsnorm.default(
                qc, kc, vc, q_w, k_w, num_q, num_kv, head_dim, 1e-6
            )
...
def bench_routing_post_topk(B, K, provider):
    ...
    if provider == "cuda":
        fn = lambda: torch.ops.sgl_kernel.gemma4_routing_post_topk.default(
            logits, ids, scale
        )
    else:
...
def bench_fused_routing(T, E, K, provider):
    ...
    if provider == "cuda":
        fn = lambda: torch.ops.sgl_kernel.gemma4_fused_routing.default(gating, scale, K)
    else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Gemma4 fused ops: CUDA vs Triton benchmark")
    print("=" * 60)
